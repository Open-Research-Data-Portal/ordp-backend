...
class DatasetArchiveRequest(models.Model):
    class Status(models.TextChoices):
        PENDING = "pending"
        APPROVED = "approved"
        REJECTED = "rejected"
    ...
    class ReasonCategory(models.TextChoices):
        ...
        PRIVACY_OR_SENSITIVE = "privacy_or_sensitive", "Privacy or Sensitive Data"
        LOW_QUALITY = "low_quality", "Low Quality"
        SUPERSEDED = "superseded", "Superseded by a Newer Dataset"
        NO_LONGER_RELEVANT = "no_longer_relevant", "No Longer Relevant"
        OTHER = "other", "Other"

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    dataset = models.ForeignKey(
        "datasets.Dataset", on_delete=models.SET_NULL, null=True, related_name="archive_requests"
    )
    dataset_title = models.CharField(max_length=255, blank=True)
    requested_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="+")
...
class ArchiveRequestVote(models.Model):
    class Vote(models.TextChoices):
        APPROVE = "approve"
        REJECT = "reject"

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    archive_request = models.ForeignKey(DatasetArchiveRequest, on_delete=models.CASCADE, related_name="votes")
    reviewer = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    vote = models.CharField(max_length=16, choices=Vote.choices)
    voted_at = models.DateTimeField(auto_now_add=True)
...
class DatasetUnarchiveRequest(models.Model):
    """Single-admin decide, not a committee vote — see decide_unarchive_request
    in datasets/services/archiving.py."""
    class Status(models.TextChoices):
        PENDING = "pending"
        APPROVED = "approved"
        REJECTED = "rejected"
    ...
    class IntendedUse(models.TextChoices):
        ...
        REPRODUCING_RESULTS = "reproducing_results", "Reproducing Results"
        PERSONAL_REFERENCE = "personal_reference", "Personal Reference"
        OTHER = "other", "Other"

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    dataset = models.ForeignKey(
        "datasets.Dataset", on_delete=models.SET_NULL, null=True, related_name="unarchive_requests"
    )
    dataset_title = models.CharField(max_length=255, blank=True)
    requested_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="+")
    intended_use = models.CharField(max_length=32, choices=IntendedUse.choices)
    reason = models.TextField()
    status = models.CharField(max_length=16, choices=Status.choices, default=Status.PENDING)
